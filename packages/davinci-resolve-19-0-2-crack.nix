{
  ...
  callPackage,
  ioPlugins ? [(callPackage ./davinci-linux-aac-codec.nix {})],
  davinci-linux-aac-codec ? (callPackage ./davinci-linux-aac-codec.nix {}),
}: let
  davinci = (
    stdenv.mkDerivation rec {
      ...
      in ''
        ...
        mkdir -p $out/{configs,DolbyVision,easyDCP,Fairlight,GPUCache,logs,Media,"Resolve Disk Database",.crashreport,.license,.LUT,IOPlugins}
        runHook postInstall
      '';
...
in
  buildFHSEnv {
    ...
    extraPreBwrapCmds = lib.optionalString studioVariant ''
      ...
      mkdir -p ~/.local/share/DaVinciResolveBinds/IOPlugins || exit 1
    '';
    ...
    extraBwrapArgs =
      ...
      ++ [
        "--bind \"$HOME\"/.local/share/DaVinciResolveBinds/IOPlugins ${davinci}/IOPlugins"
      ];
