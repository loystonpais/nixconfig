  ...
  {...} @ lib: rec {
    ...
    importDir = {
      ...
      attrs = lib.trivial.pipe (dirs // files) [
        ...
        # Removes default.nix
        (a:
          if excludeDefaultNix
          then filterAttrsByName (fileName: fileName != "default.nix") a
          else a)

        # Clean the source
        (a:
          if cleanSource
          then filterAttrs lib.sources.cleanSourceFilter a
          else a)
        ...
        (a:
          if importPaths
          then mapAttrs (fileName: path: import path) a
          else a)
      ];
    in
      attrs;
